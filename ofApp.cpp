...
void ofApp::update() {
	...
	for (int z = -600; z <= 600; z += 12) {
		...
		int deg_start = ofMap(ofNoise((z + 600) * 0.005 + ofGetFrameNum() * 0.008), 0, 1, -540, 540);
		this->setRingToMesh(this->face, this->frame, glm::vec3(0, 0, z), radius, 50, 10, deg_start, deg_start + 120);
	}
}
